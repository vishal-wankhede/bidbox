            ...
            document.querySelectorAll('.edit-filter-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    document.getElementById('editFilterForm').action =
                        `/utilities/filters/update/${id}`;
                });
            });

            document.querySelector('.add-filter-btn').addEventListener('click', function() {
                const pathSegments = window.location.pathname.split('/').filter(
                    Boolean); // removes empty parts
                const lastSegment = pathSegments[pathSegments.length - 1];

                // Check if it's a number (like /filters/3)
            ...
            document.querySelectorAll('.edit-filterValue-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    document.getElementById('editFilterValueForm').action =
                        `/utilities/filter_values/update/${id}`;
                });
            });
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <a href="{{ route('utilities.filters.changeStatus', $filter->id) }}"
                                                    class="btn btn-sm {{ $filter->status ? 'btn-outline-danger' : 'btn-outline-success' }}">
